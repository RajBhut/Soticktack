...
const [showpopup , setShowpopup] = useState(false);
  ...
  return (<>
    <div className='h-screen w-screen bg-slate-400 flex justify-center items-center '>
      ...
      <button className=' p-1 w-20px h-20px bg-slate-500 text-white border' onClick={() => {if(bordSize < 8 ){setBordSize(bordSize+1)}}}>Increase Board Size</button>
    <button className=' p-1 w-20px h-20px text-slate-500 bg-white' onClick={() => {if(bordSize > 3){setBordSize(bordSize-1)}}}>Decrease Board Size</button>
    <button className=' p-1 text-cyan-50 bg-zinc-500' onClick={
      ()=>{setShowpopup(true)}
    }>Start Game</button>
      </div>
    </div>
